...
int svmrun(svm_model* svmmodel)
{
	...
	int min_index;
	int i,index;
	FILE *fp			= fopen("vector.pca","r");
	FILE *fp_restore 	= fopen("emotions.train.pca.range","r");
...
void predict(svm_model* svmmodel, struct svm_node *x, double predict_label)
{
	...
	for(j = 0; j < nr_class; j++) printf("%d:%g ", j+1, 100*prob_estimates[j]);
	predict_label = svm_predict_probability(svmmodel,x,prob_estimates);
	printf("done");
	printf("%g\n",predict_label);
	printf("Scaled values (from predict): \n");
