...
{ 
  ...
  bool gen,rsize=true;
  cv::Rect R;
  //if "_frame" is negative valued or if fpd is exceeded, perform face detection = FIRST INIT
  ...
  //this else happens to continue tracking? "gen" implies shape generation?
  else{R = this->ReDetect(gray_); gen = false;}
  // if face is not found, increase frame count since last face-detected
